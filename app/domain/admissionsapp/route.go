...
func Routes(app *web.App, cfg Config) {
	...
	app.HandlerFunc(http.MethodPost, version, "/admissions/events", api.createEvent, authen, ruleManageEvents, transaction)
	...
	app.HandlerFunc(http.MethodPut, version, "/admissions/events/{event_id}", api.updateEvent, authen, ruleManageEvents, transaction)
	...
	app.HandlerFunc(http.MethodGet, version, "/admissions/campaigns", api.queryCampaigns, authen, ruleRead)
	app.HandlerFunc(http.MethodGet, version, "/admissions/campaigns/{campaign_id}", api.queryCampaignByID, authen, ruleRead)
	app.HandlerFunc(http.MethodGet, version, "/admissions/campaigns/{campaign_id}/audit-events", api.queryCampaignAuditEvents, authen, ruleRead)
	app.HandlerFunc(http.MethodGet, version, "/admissions/communications", api.queryCommunications, authen, ruleRead)
	app.HandlerFunc(http.MethodGet, version, "/admissions/communications/{communication_id}", api.queryCommunicationByID, authen, ruleRead)
